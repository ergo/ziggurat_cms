...
from ziggurat_cms.node_types.blog_node.models import BlogNode
from ziggurat_cms.node_types.blog_node.schemes import BlogNodeCreateSchema
from ziggurat_cms.node_types.blog_node.schemes import BlogNodeGETSchema
...
CONTEXT_TYPE = 'ziggurat_cms.node_types.blog_node.security.BlogNodeContext'
...
@view_defaults(route_name='api_object', renderer='json',
               permission='view_api',
               match_param='object=zigguratcms-blog-nodes')
class BlogNodesAPIView(BaseView):
    """
    Views for nageNode type resources
    """

    def __init__(self, request):
        super(BlogNodesAPIView, self).__init__(request)
        # self.shared = BlogNodesShared(request)
        self.resources_shared = ResourcesShared(request)

    @view_config(match_param='object=zigguratcms-blog-nodes',
                 context=CONTEXT_TYPE, permission='view_api')
    def get(self):
        resource = self.request.context.resource
        schema = BlogNodeGETSchema()
