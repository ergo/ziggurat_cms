...
from ziggurat_cms.services.user_permission import \
...
class UsersShared(object):
    ...
    def collection_list(self, tenant_pkey, page=1, filter_params=None):
    ...
    def user_get(self, user_uuid):
        request = self.request
        user = UserService.by_uuid(safe_uuid(user_uuid),
                                   db_session=request.dbsession)
        if not user:
