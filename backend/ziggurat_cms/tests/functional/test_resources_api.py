...
class TestFunctionalAPIResources(object):
    def test_user_permission_wrong_resource(self, full_app, sqla_session):
        with session_context(sqla_session) as session:
            ...
            user = create_user({'public_user_name': 'aaaa', 'public_email': 'foo'},
                               sqla_session=session)
            resource = create_page_node({'resource_name': 'entry-x', 'description': 'x'},
                                    sqla_session=session)
    ...
    def test_user_permission_add_proper(self, full_app, sqla_session):
        with session_context(sqla_session) as session:
            application, admin, token = set_default_app_data(session)
            user = create_user(
                {'public_user_name': 'aaaa', 'public_email': 'foo'}, sqla_session=session)
            resource = create_page_node(
                {'resource_name': 'entry-x', 'description': 'x'}, sqla_session=session)
