...
from ziggurat_cms.tests.utils import (
    create_page_node,
    session_context,
    create_admin,
)
...
@pytest.mark.usefixtures('full_app', 'with_migrations', 'clean_tables',
                         'sqla_session')
class TestFunctionalAPIPageNodes(object):
    def test_wrong_token(self, full_app):
        url_path = '/api/0.1/zigguratcms-page_nodes'
        headers = {str('x-zigguratcms-auth-token'): str(
            'aaaaaaaa-aaaa-aaaa-aaaaaaaaaaaaaaaa')}
        full_app.post(url_path, {}, status=403, headers=headers)

    def test_page_nodes_list(self, full_app, sqla_session):
        with session_context(sqla_session) as session:
            application, admin, token = set_default_app_data(session)
            for x in range(1, 51):
                create_page_node({'resource_name': 'entry-x{}'.format(x),
                              'description': 'x{}'.format(x)},
                             sqla_session=session)
                create_page_node({'resource_name': 'entry-y{}'.format(x),
                              'description': 'y{}'.format(x)},
                             sqla_session=session)

        url_path = '/api/0.1/resources'
        headers = {str('x-zigguratcms-auth-token'): str(token)}
    ...
    def test_entry_create(self, full_app, sqla_session):
        ...
        assert entry_dict['description'] == response.json['description']

    def test_entry_patch(self, full_app, sqla_session):
        with session_context(sqla_session) as session:
            application, admin, token = set_default_app_data(session)
            entry = create_page_node({'resource_name': 'entry-x',
                                  'description': 'x'},
                                 sqla_session=session)

        url_path = '/api/0.1/zigguratcms-page_nodes/{}'.format(entry.resource_id)
        headers = {str('x-zigguratcms-auth-token'): str(token)}
        entry_dict = {
            'resource_id': -9,
    ...
    def test_entry_delete_branch(
        ...
        url_path = '/api/0.1/zigguratcms-page_nodes/{}'.format(1)
        headers = {str('x-zigguratcms-auth-token'): str(token)}
        full_app.delete_json(url_path, status=200, headers=headers)
        result = tree_service.from_parent_deeper(None, db_session=sqla_session)
    ...
    def test_entry_patch_order_same_branch(self, full_app, sqla_session):
        ...
        url_path = '/api/0.1/zigguratcms-page_nodes/{}'.format(-2)
        headers = {str('x-zigguratcms-auth-token'): str(token)}
        entry_dict = {
            'ordering': 3
        }
    ...
    def test_entry_patch_order_same_branch_nested(
        ...
        url_path = '/api/0.1/zigguratcms-page_nodes/{}'.format(node_id)
        headers = {str('x-zigguratcms-auth-token'): str(token)}
        entry_dict = {
            'ordering': position
        }
    ...
    def test_entry_patch_order_upper_branch_no_order(
        ...
        url_path = '/api/0.1/zigguratcms-page_nodes/{}'.format(12)
        headers = {str('x-zigguratcms-auth-token'): str(token)}
        entry_dict = {
            'parent_id': 1
        }
