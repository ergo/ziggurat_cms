# -*- coding: utf-8 -*-
from __future__ import absolute_import, unicode_literals, print_function

import pytest
from six.moves.urllib import parse

from ziggurat_cms.tests.utils import (
    set_default_app_data,
    create_user,
    session_context)


@pytest.mark.usefixtures('full_app', 'with_migrations', 'clean_tables',
                         'sqla_session')
class TestFunctionalAPIUsers(object):

    def test_malformed_token(self, full_app):
        url_path = '/api/0.1/users'
        headers = {str('x-zigguratcms-auth-token'): str('test')}
        full_app.post(url_path, {}, status=400, headers=headers)

    def test_wrong_token(self, full_app, sqla_session):
        import uuid
        with session_context(sqla_session) as session:
            application, admin, token = set_default_app_data(session)

        url_path = '/api/0.1/users'
        headers = {str('x-zigguratcms-auth-token'): str(uuid.uuid4())}
        full_app.post(url_path, {}, status=403, headers=headers)

    def test_users_list(self, full_app, sqla_session):
        with session_context(sqla_session) as session:
            application, admin, token = set_default_app_data(session)
            create_user(application.resource_id,
                        {'public_user_name': 'test2',
                         'public_email': 'test@test.local2'},
                        sqla_session=session)

        url_path = '/api/0.1/users'
        headers = {str('x-zigguratcms-auth-token'): str(token)}
        response = full_app.get(url_path, status=200, headers=headers)
        items = response.json
        assert len(items) == 2

    def test_users_filtering(self, full_app, sqla_session):
        with session_context(sqla_session) as session:
            application, admin, token = set_default_app_data(session)
            create_user(application.resource_id,
                        {'public_user_name': 'test2',
                         'public_email': 'test@test.local2'},
                        sqla_session=session)
            create_user(application.resource_id,
                        {'public_user_name': 'foo',
                         'public_email': 'test2@test.local2'},
                        sqla_session=session)
            create_user(application.resource_id,
                        {'public_user_name': 'barbaz',
                         'public_email': 'test3@test.local2'},
                        sqla_session=session)

        url_path = '/api/0.1/users?user_name_like=bar'
        headers = {str('x-zigguratcms-auth-token'): str(token)}
        response = full_app.get(url_path, status=200, headers=headers)
        items = response.json
        assert items[0]['public_user_name'] == 'barbaz'

    def test_user_create_no_json(self, full_app, sqla_session):
        with session_context(sqla_session) as session:
            application, admin, token = set_default_app_data(session)

        url_path = '/api/0.1/users'
        headers = {str('x-zigguratcms-auth-token'): str(token)}
        full_app.post_json(url_path, status=422, headers=headers)

    def test_user_create_bad_json(self, full_app, sqla_session):
        with session_context(sqla_session) as session:
            application, admin, token = set_default_app_data(session)

        url_path = '/api/0.1/users'
        headers = {str('x-zigguratcms-auth-token'): str(token)}
        user_dict = {'xxx': 'yyy'}
        response = full_app.post_json(url_path, user_dict, status=422,
                                      headers=headers)
        required = ['public_user_name', 'public_email', 'password']
        assert sorted(required) == sorted(response.json.keys())

    def test_user_create(self, full_app, sqla_session):
        with session_context(sqla_session) as session:
            application, admin, token = set_default_app_data(session)

        url_path = '/api/0.1/users'
        headers = {str('x-zigguratcms-auth-token'): str(token)}
        user_dict = {
            'id': -9999,
            'public_user_name': 'some-new_user',
            'password': 'new_password',
            'public_email': 'email@foo.bar'
        }
        response = full_app.post_json(url_path, user_dict, status=200,
                                      headers=headers)
        assert response.json['id'] > 0
        assert user_dict['public_user_name'] == response.json['public_user_name']
        assert user_dict['public_email'] == response.json['public_email']
        assert 'last_login_date' in response.json
        assert 'registered_date' in response.json
        assert response.json['status'] == 1

    def test_user_duplicate(self, full_app, sqla_session):
        with session_context(sqla_session) as session:
            application, admin, token = set_default_app_data(session)

        url_path = '/api/0.1/users'
        headers = {str('x-zigguratcms-auth-token'): str(token)}
        user_dict = {
            'id': -9999,
            'public_user_name': 'test',
            'password': 'new_password',
            'public_email': 'email@foo.bar'
        }
        response = full_app.post_json(url_path, user_dict, status=422,
                                      headers=headers)
        assert response.json['public_user_name']

    def test_user_patch(self, full_app, sqla_session):
        with session_context(sqla_session) as session:
            application, admin, token = set_default_app_data(session)
            user = create_user(
                application.resource_id,
                {'public_user_name': 'testX',
                 'public_email': 'testX@test.local'},
                sqla_session=session)

        url_path = '/api/0.1/users/{}'.format(user.uuid)
        headers = {str('x-zigguratcms-auth-token'): str(token)}
        user_dict = {
            'id': -9,
            'uuid': -9,
            'user_name': 'some-new_userCHANGED',
            'public_user_name': 'some-new_userCHANGED',
            'public_email': 'email@fooCHANGED.bar'
        }
        response = full_app.patch_json(url_path, user_dict, status=200,
                                       headers=headers)
        assert response.json['id'] == user.id
        assert 'testX' == response.json['public_user_name']
        assert user_dict['public_email'] == response.json['public_email']

    def test_user_delete(self, full_app, sqla_session):
        with session_context(sqla_session) as session:
            application, admin, token = set_default_app_data(session)
            user = create_user(
                application.resource_id,
                {'public_user_name': 'testX',
                 'public_email': 'testX@test.local'},
                sqla_session=session)
        url_path = '/api/0.1/users/{}'.format(user.id)
        headers = {str('x-zigguratcms-auth-token'): str(token)}
        full_app.delete_json(url_path, status=200, headers=headers)


@pytest.mark.usefixtures('full_app', 'with_migrations', 'clean_tables',
                         'sqla_session')
class TestFunctionalAPIUsersPermissions(object):
    def test_permission_add(self, full_app, sqla_session):
        with session_context(sqla_session) as session:
            application, admin, token = set_default_app_data(session)
            user = create_user(
                application.resource_id,
                {'public_user_name': 'testX',
                 'public_email': 'testX@test.local'},
                sqla_session=session)

        url_path = '/api/0.1/users/{}/permissions'.format(user.id)
        headers = {str('x-zigguratcms-auth-token'): str(token)}
        permission = {
            'perm_name': 'root_administration',
        }
        assert not list(user.permissions)
        full_app.post_json(url_path, permission, status=200, headers=headers)
        sqla_session.expire_all()
        print(user.permissions)
        assert user.permissions[0].perm_name == 'root_administration'

    def test_permission_delete_not_found(self, full_app, sqla_session):
        with session_context(sqla_session) as session:
            application, admin, token = set_default_app_data(session)
            user = create_user(
                application.resource_id,
                {'public_user_name': 'testX',
                 'public_email': 'testX@test.local'},
                sqla_session=session)

        url_path = '/api/0.1/users/{}/permissions'.format(user.id)
        headers = {str('x-zigguratcms-auth-token'): str(token)}
        permission = {
            'perm_name': 'c',
        }
        full_app.delete(url_path, permission, status=404, headers=headers)

    def test_permission_delete(self, full_app, sqla_session):
        with session_context(sqla_session) as session:
            application, admin, token = set_default_app_data(session)
            user = create_user(
                application.resource_id,
                {'public_user_name': 'testX',
                 'public_email': 'testX@test.local'},
                permissions=['root_administration', 'admin_panel'],
                sqla_session=session)

        url_path = '/api/0.1/users/{}/permissions'.format(user.id)
        headers = {str('x-zigguratcms-auth-token'): str(token)}
        permission = {
            'perm_name': 'root_administration',
        }
        qs = parse.urlencode(permission)
        full_app.delete('{}?{}'.format(url_path, qs),
                        status=200, headers=headers)
        sqla_session.expire_all()
        assert user.permissions[0].perm_name == 'admin_panel'
