...
def _get_session(settings):
    from ziggurat_cms.models import get_engine, get_session_factory
...
def base_app(request, app_settings):
    from ziggurat_cms import main
    from webob.multidict import MultiDict
    from pyramid.request import apply_request_extensions
    app = main({}, **app_settings)
    app_request = testing.DummyRequest(base_url='https://ziggurat_cms.com')
...
def with_migrations(app_settings):
    ...
    alembic_cfg = Config()
    alembic_cfg.set_main_option(
        'script_location', 'ziggurat_cms:migrations')
    alembic_cfg.set_main_option('sqlalchemy.url',
                                app_settings['sqlalchemy.url'])
    command.upgrade(alembic_cfg, 'head')
