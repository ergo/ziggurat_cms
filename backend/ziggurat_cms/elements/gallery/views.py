...
from ziggurat_cms.elements.gallery import ELEMENT_CONFIG
from ziggurat_cms.elements.gallery.models import ZigguratCMSGallery
...
@view_defaults(route_name='api_object', renderer='json',
               permission='view_api',
               match_param=('object=zigguratcms-gallery-elements',))
class GalleryElementAPIView(BaseView):
    """
    Views for Gallery element
    """

    def __init__(self, request):
        super(GalleryElementAPIView, self).__init__(request)
    ...
    @view_config(request_method='POST', route_name='api_object_relation',
                 match_param=('relation=zigguratcms-gallery-elements',
                              'object=resources'))
    def post(self):
        resource = self.request.context.resource
        ...
        node_element = ZigguratCMSGallery()
    ...
    @view_config(route_name='api_object_relation', renderer='json',
                 ...
                 match_param=(
                         'object=zigguratcms-gallery-elements',
                         'relation=images'))
    def images_get(self):
    ...
    def image_post(self):
        ...
        upload_dir = os.path.join('public', 'zigguratcms-gallery')
        ...
        ElementUploadImageService.save_resized_image(
            img, miniature_parsed_name.filename, absolute_upload_dir,
            max_size=max_size)
    ...
    @view_config(route_name='api_object', renderer='json',
                 ...
                 match_param='object=zigguratcms-gallery-elements-images')
    def image_delete(self):
        image = self.request.context.image
        root_dir = self.request.registry.settings['upload.root_dir']
        ElementUploadImageService.delete_file(root_dir, image.upload_path,
                                              image.filename)
        ElementUploadImageService.delete_file(root_dir, image.upload_path,
                                              image.miniature_filename)
    ...
    @view_config(route_name='api_object', renderer='json',
                 ...
                 match_param='object=zigguratcms-gallery-elements-images')
    def image_patch(self):
        image = self.request.context.image
        json_body = self.request.json_body
        image.populate_obj(json_body, include_keys=['name', 'description'])
        schema = UploadImageSchema()
        return schema.dump(image).data
