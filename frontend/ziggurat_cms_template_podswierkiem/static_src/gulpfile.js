...
global.config = {
    ...
    rootDir: process.cwd(path.join('src', 'ziggurat_cms_template_podswierkiem')),
    name: 'ziggurat_cms_template_podswierkiem'
};
...
function copy_images() {
    return gulp.src(path.join(global.config.rootDir, 'images', '**'))
        .pipe(gulp.dest(path.join(global.config.destinationDirectory,
            global.config.name, 'images')));
}
function copy_bower() {
